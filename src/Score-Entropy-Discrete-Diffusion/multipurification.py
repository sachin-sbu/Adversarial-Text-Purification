...
def main():
    ...
    fp = open("/home/sdanisetty/projects/advpur/repos/ag_bert", "r")
    i = 0
    og_texts = []
    adv_texts = []
    for line in fp:
    ...
    for i in pbar:
        ...
        timesteps = [0.2]
        score = 0
        scores = {"World":0, "Sports":0, "Business":0, "Sci/Tech":0}
        steps = 2
        print("steps : ", steps)
        difftext = adv_text
        for j in range(steps):
            t = 0.1
            noised, noisy_tokens = diffusion.add_noise(difftext, t)
            ...
            # print('purified_text at ' + str(j) + " :", denoised)
            # print('purified_label at ' + str(j) + " :", label)
            # print()
            if og_label == label:
                score += 1
            else:
