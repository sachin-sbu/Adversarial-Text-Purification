...
def main():
    ...
    input_locs = list(range(len(prefix_ids))) + list(range(30-len(suffix_ids), 30))
    ...
    sampling_fn = sampling.get_pc_sampler(
        graph, noise, (args.batch_size, 30), 'analytic', args.steps, device=device, proj_fun=proj_fun
    )
